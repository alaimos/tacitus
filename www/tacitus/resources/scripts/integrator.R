...
main <- function (args) {
  ...
  write.status(args$status, tryCatch({
    ...
    selections <- lapply(1:length(config.data$selections), function (i) {
      se <- read.selection(data.file=config.data$selections[[i]]$data, 
                           metadata.file=config.data$selections[[i]]$metadata, 
                           na.strings=config.data$na.strings)
      cat("...",i,sep="")
      return (se)
    })
    cat("...OK\n")
